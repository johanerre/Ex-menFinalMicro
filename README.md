...
void setup() {
    ...
    // ... configuración motores, servos ...
}
```

### Comunicación Serial (Arduino como esclavo):
- **Serial.begin(9600)**: Inicializa puerto hardware
  - **Velocidad**: 9600 baudios (debe coincidir con ESP32)
  - **Pines fijos**: Pin 0 (RX), Pin 1 (TX) - NO se pueden cambiar
  - **Función dual**: Recibe comandos PS4 Y envía datos IMU
...
```cpp
void loop() {
    // ---- Lectura de datos del control PS4 (desde ESP32) ----
    ...
    // ---- Lectura del IMU cada 100ms ----
    unsigned long tiempoActual = millis();
    if (tiempoActual - tiempoIMU >= INTERVALO_IMU) {
        tiempoIMU = tiempoActual;
        leerIMU();  // Lee IMU y envía al ESP32
    }
}
```

**Comunicación bidireccional**:
1. **Recibe comandos del ESP32**: Datos del PS4 en formato `x,y,sq,cr,ci,tr\n`
2. **Controla hardware**: Mueve motores y servos según comandos
3. **Envía datos IMU**: Cada 100ms envía `pitch,roll\n` de vuelta al ESP32
...
**Función**: Envía los ángulos por serial al ESP32 en formato: `pitch,roll` (ejemplo: `5.12,3.45`)

**IMPORTANTE**: Este es el MISMO puerto que recibe comandos del PS4, por eso NO debe haber `Serial.print()` de debug.
...
2. **Serial2.begin(9600, SERIAL_8N1, RXD2, TXD2)**:
   ...
   - **SERIAL_8N1**: 8 bits de datos, sin paridad, 1 bit de parada
   - Usa pines 16 (RX) y 17 (TX)
   - **Pines configurables**: El ESP32 permite elegir qué pines usar
...
## Loop - Parte 1: Lectura del IMU (UART2 desde Arduino)
```cpp
while (Serial2.available()) {
    char c = (char)Serial2.read();
...
1. **Serial2.available()**: Verifica si hay datos esperando del Arduino
2. **Serial2.read()**: Lee un carácter del UART2
3. **if (c == '\n')**: Si es fin de línea
   - Marca que hay una línea completa
   - Sale del while
4. **else if (c != '\r')**: Si no es retorno de carro
...
## Loop - Parte 2: Procesamiento y reenvío del IMU
```cpp
if (lineReadyUART2) {
    if (parseTwoFloats(bufUART2, dato1_uart2, dato2_uart2)) {
...
## Loop - Parte 3: Control PS4 y envío al Arduino
```cpp
if (PS4.isConnected()) {
    int x = PS4.LStickX();      // -127 a 127
    int y = PS4.LStickY();
...
# CONFIGURACIÓN MAESTRO-ESCLAVO-MAESTRO
---
...
### Puerto serial:
- **PORT = "COM4"**: Puerto donde está conectado el ESP32 (cambiar según sistema)
- **BAUD = 115200**: Velocidad de comunicación (DEBE coincidir con ESP32)
...
### Función:
Establece conexión USB con el ESP32.

### Paso a paso:

1. **serial.Serial()**: Abre el puerto serial
   - **PORT**: Puerto COM (Windows) o /dev/ttyUSB0 (Linux)
   - **BAUD**: Velocidad en baudios (115200, igual que ESP32)
   - **timeout=1**: Espera máximo 1 segundo por datos

2. **time.sleep(2)**: Espera 2 segundos
   - El ESP32 puede resetear al abrir conexión serial
